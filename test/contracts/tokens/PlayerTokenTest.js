...
contract('PlayerToken', function (accounts) {
  ...
  const decimals = 18
  const initialAmount = new BN(10).mul(new BN(10).pow(new BN(6 + decimals)))
  const symbol = 'FLMS'

  let init = async () => {
    tokenContract = await TokenContract.new(initialAmount, 2374, symbol, 'Lionel Messi Token', 'football', {from: creatorAccount})
  }
